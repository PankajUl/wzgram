...
def _parse_proxy_link(link: str) -> Proxy:
    ...
    # `urlsplit` hands back the userinfo still percent-encoded, so a credential
    #  holding `@` or `:` - which a URL forces the writer to escape - would reach
    #  the proxy as the escape text itself. The `tg://socks?user=&pass=` form goes
    #  through `parse_qs`, which decodes, so decoding here is also what keeps the
    #  two spellings of one credential equal.
    return _build_dialed_proxy(
        scheme=scheme,
